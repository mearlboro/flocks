...
def process_space(
        ...
        L: float,
        topology: EnumBounds,
        relative_to: str = 'centre_of_mass',
    ...
    dict of numpy arrays
      ...
      'std_dist_cmass': (T,)      standard deviation of distance relative to centre
    """
    if relative_to not in ( 'centre_of_mass', 'centre_of_space' ):
        raise ValueError("process_space: relative_to param must be centre_of_mass or centre_of_space")

    (T, N, D) = Xt.shape

    cmass = [ centre_of_mass(X, L, topology) for X in Xt ]

    if relative_to == 'centre_of_mass':
        rel = [ relative_positions(X, c, L, topology) for (X, c) in zip(Xt, cmass) ]
    else:
        c = np.array([ l/2 ] * D)
        rel = [ relative_positions(X, c, L, topology) for X in Xt ]
    ...
    m['std_dist_cmass'] = np.std( dist, axis = 1)
...
def process_angles(At: np.ndarray, Vt: np.ndarray = None, v: float = 1.0) -> Dict[str, np.ndarray]:
    ...
    dict of numpy arrays
        ...
        'std_angle':   (T,) std dev of angle
        'abs_avg_vel': (T,) absolute average velocity
        'std_abs_vel': (T,) std dev of absolute average velocity
    """
    (T, N) = At.shape
    ...
    m['std_angle'] = np.array([ vec_to_ang(v) for v in std_vel ])

    m['avg_abs_vel'] = np.linalg.norm(avg_vel, 2, axis = 1) / v
    m['std_abs_vel'] = np.linalg.norm(std_vel, 2, axis = 1) / v
